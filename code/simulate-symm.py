...
from symm_error import *
# from symm_error2 import errorMis2, errorMis1, errorMis3, errorMis5, errorMis4, errorMis6, errorMis7, derivMis
from strings import errorMis7, derivMis
...
for p_1 in p_1_opt:
    ...
    for x_bar_1 in options:
        ...
        error_val = errorMis7(p_1,x_bar_1)
        # error_val = error45(p_1, x[:,1],x_bar_1, g0, g1)
        # error_val = error7(p_1, x_bar_1,g0, g1)
        # deriv = deriv5(p_1, x_bar_1,g0, g1)
        # deriv = deriv7(p_1, x_bar_1,g0, g1)
        deriv = derivMis(p_1, x_bar_1)
        ...
        deriv_temp = pd.DataFrame({'p_0': p_0, 'p_1': round(p_1,3), 'x_bar_0': x_bar[0], 'x_bar_1' : round(x_bar[1],3), 'deriv': deriv}, index=[0])
...
#     g0 = gammas[:,0]
#     g1 = gammas[:,1]
#     t1 = test1(p, 0.2, g0, g1) 
#     t2 = test2(p, 0.2, g0, g1)
#     print(t1, t2)
    # assert
#     # x = np.zeros((num_ind, 2))
#     x = np.random.binomial(1, p, size=num_ind)
#     y_error2 = [error2(p, props, x, g0, g1) for props in options]
#     # y_error2 = [error4(p, props, g0, g1) for props in options]
#     y_error5 = [error6(p, props, g0, g1) for props in options]
#     ax[index,1].plot(options, y_error2)
#     ax[index,2].plot(options, y_error5)
#     index +=1
#     # assert (np.abs(np.array(y_error5) - np.array(y_error2)) < 0.001).all()
