            ...
            for x_bar_0 in options:
                for x_bar_1 in options:
                    for x_bar_2 in options:
                        ...
                        c_i = np.zeros(num_ind).reshape(num_ind,1)
                        ...
                        # error = (np.square(bpMinb)).mean()

                        error = MSE(x, x_bar, c_i, gammas)
                        # error_anal = anal_error2(x, x_bar, gammas)
                        # error_test = test(x, x_bar, gammas)

                        # error2 = test_sol(x, xb, yb, zb)
                        # error_cov = sol_cov(x, x_bar, gammas)
                        error_cov = sol_cov3(x, x_bar, gammas, beta_ate)

                        diff_cov = np.abs(error - error_cov)
                        # diff2 = np.abs(error - error_anal)
                        # diff2 = np.abs(error - error3)

                        print(error, error_cov, diff_cov)
                        # assert diff < 0.001
                        # assert diff2 < 0.001
                        if diff_cov > max_diff:
                            max_diff = diff_cov
                            print(max_diff)
