...
\begin{enumerate}
	...
	\item  Suppose a physician can only see see ATE and some measure of representativeness. They have prior
		$\bar{\beta}$ and  $\beta_{ATE} = (1/N) \sum \beta_{j}$.
	...
	\item need model for betas related to each other based on $x's$.
		...
		\begin{align*}
			\beta(x_j) = x_j \gamma
		\end{align*} 
		If you knew the gamma, then you would know beta for any given patient.
	\item You don't observe gamma, you instead observe:
	...
	\item For other patients, need to solve
		\begin{align*}
			\E(x_i \gamma | \bar x \gamma = \beta_{ATE})
		\end{align*} 
			...
			\item
				\begin{align*}
					\beta_{i,post} & = \E (x_i \gamma | \bar x \gamma = \beta_{ATE})                                                                        \\
								   & = \E((x_i - c \bar{x}_j) \gamma | \bar{x} \gamma = \beta_{ATE}) + c \E(\bar{x} \gamma | \bar{x} \gamma = \beta_{ATE}) \\
								   & = \E((x_{i} - c \bar{x}) \gamma | \bar{x} \gamma = \beta_{ATE}) + c \beta_{ATE}                                       \\
				\end{align*}
				For any constant $c$. \\
				Choose $c$ so that 
				\begin{align*}
					\Cov((x_{i} - c \bar{x})\gamma, \bar{x} \gamma) = 0
				\end{align*} 
				...
				So then
				\begin{align*}
					(x_i - c \bar{x}) \E(\gamma) + c_i \beta_{ATE} = x_i \E(\gamma) + c_i (\beta_{ATE} - \bar{x} \E(\gamma))
				\end{align*} 
				(c depends on $x_i$)
				...
				The key question is how much adjustment you do which depends on $"c"$. We choose $c$ to solve:

				\begin{align*}
                          & \Cov ((x_i - c \bar{x}) \gamma, \bar{x} \gamma) = 0                          \\
				 \iff     & \Cov(x_i \gamma, \bar{x} \gamma) -c \Cov(\bar{x} \gamma, \bar{x} \gamma) = 0 \\
